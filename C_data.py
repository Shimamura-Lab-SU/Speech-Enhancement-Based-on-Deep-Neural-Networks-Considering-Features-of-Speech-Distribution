...
class create_batch:
    ...
    def __init__(self, clean_data, noisy_data, batches):
        ...
        # 正規化
        def normalize(data):
            return (1. / 512.) * data  # [-32768 ~ 32768] -> [-1 ~ 1]

        # データの整形
        self.clean = np.expand_dims(normalize(clean_data),axis=1)
        self.noisy = np.expand_dims(normalize(noisy_data),axis=1)
